<?	
	...
	$no=$_GET['no'];
	if(!$no)
		echo "<script>location.href=\"member2.php?url=$imgurl\"</script>";
	else{
		include ("dblib.php");
		$conn=mysqli_connect($db_host,$db_user,$db_passwd,$db_name);
		if($conn)echo "error!";
		$result=mysqli_query($conn,"update as_user2 set pfimg='$imgurl' where no=$no");
		echo "<script>location.href=\"member2.php\"</script>";
	}
?>